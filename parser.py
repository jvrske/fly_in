...
class Parser():
    """parses drone network map files into structured config dicts"""

    @staticmethod
    def parser(map_txt: str) -> dict:
        """parse the map file and return a structured config dict"""
        result: dict[str, Any] = {}
        ...
        try:
            ...
            def parse_hub(value: str, i: int) -> dict:
                """return a dict with name, x, y and raw metadata string"""
            ...
            def parse_hub_metadata(hub: dict) -> dict:
                """parse hub metadata string and return a structured dict"""
                line = hub["line"]
            ...
            def apply_metadata(hub: dict) -> dict:
                """strip temporary fields and apply parsed metadata
                to the hub"""
            ...
            if result["end_hub"].get("metadata").get("max_drones") \
                    < result["nb_drones"]:
                raise ParserError(f"Line {line_numbers['end_hub']}: \
Max drones of end_hub couldn't be smaller than nb_drones")

            parsed_hubs = []
            for val, line_num in result.get("hub", []):
            ...
            hub_names = [result["start_hub"]["name"],
                         result["end_hub"]["name"]] + \
                        [h["name"] for h in result["hub"]]
            seen_names: set[str] = set()
            for name in hub_names:
                if name in seen_names:
                    raise ParserError(f"Duplicate hub name: '{name}'")
                seen_names.add(name)

            def parse_connections_meta(conn_meta: str, line: int) -> dict:
                """parse metadata block from a connection line"""
            ...
            def parse_connections(value: list) -> list:
                """parse and validate all connections, return list of dicts"""
                parsed = []
                seen = set()
